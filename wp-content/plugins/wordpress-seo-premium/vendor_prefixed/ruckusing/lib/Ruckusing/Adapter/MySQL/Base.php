 ...
 * @link      https://github.com/ruckus/ruckusing-migrations
 */
class Ruckusing_Adapter_MySQL_Base extends \YoastSEO_Vendor\Ruckusing_Adapter_Base implements \YoastSEO_Vendor\Ruckusing_Adapter_Interface
{
    /**
     * Name of adapter
     *
    ...
    {
        $types = array('primary_key' => array('name' => 'integer', 'limit' => 11, 'null' => \false), 'string' => array('name' => "varchar", 'limit' => 255), 'text' => array('name' => "text"), 'tinytext' => array('name' => "tinytext"), 'mediumtext' => array('name' => 'mediumtext'), 'integer' => array('name' => "int", 'limit' => 11), 'tinyinteger' => array('name' => "tinyint"), 'smallinteger' => array('name' => "smallint"), 'mediuminteger' => array('name' => "mediumint"), 'biginteger' => array('name' => "bigint"), 'float' => array('name' => "float"), 'decimal' => array('name' => "decimal", 'scale' => 0, 'precision' => 10), 'datetime' => array('name' => "datetime"), 'timestamp' => array('name' => "timestamp"), 'time' => array('name' => "time"), 'date' => array('name' => "date"), 'binary' => array('name' => "blob"), 'tinybinary' => array('name' => "tinyblob"), 'mediumbinary' => array('name' => "mediumblob"), 'longbinary' => array('name' => "longblob"), 'boolean' => array('name' => "tinyint", 'limit' => 1), 'enum' => array('name' => "enum", 'values' => array()), 'uuid' => array('name' => "char", 'limit' => 36), 'char' => array('name' => "char"));
        return $types;
    }
    /**
...
//free up mysqli
        ...
        if ($this->isError($res)) {
            throw new \YoastSEO_Vendor\Ruckusing_Exception(\sprintf("Error executing 'query' with:\n%s\n\nReason: %s\n\n", $queries, $this->conn->error), \YoastSEO_Vendor\Ruckusing_Exception::QUERY_ERROR);
        }
...
//free up mysqli
    ...
    public function indexes($table_name)
    {
        ...
        foreach ($result as $row) {
            ...
            $indexes[] = array('name' => $row['Key_name'], 'unique' => (int) $row['Non_unique'] == 0 ? \true : \false);
        }
        return $indexes;
    }
    /**
    ...
    public function type_to_sql($type, $options = array())
    {
        ...
        if ($type == "decimal") {
            ...
            if ($precision != null) {
                ...
                if ($scale) {
                    throw new \YoastSEO_Vendor\Ruckusing_Exception("Error adding float column: precision cannot be empty if scale is specified", \YoastSEO_Vendor\Ruckusing_Exception::INVALID_ARGUMENT);
                }
            }
            //precision
        } elseif ($type == "enum") {
            if (empty($values)) {
                throw new \YoastSEO_Vendor\Ruckusing_Exception("Error adding enum column: there must be at least one value defined", \YoastSEO_Vendor\Ruckusing_Exception::INVALID_ARGUMENT);
            } else {
    ...
    public function add_column_options($type, $options)
    {
        ...
        if (\array_key_exists('unsigned', $options) && $options['unsigned'] === \true) {
            $sql .= ' UNSIGNED';
        }
        ...
        if (\array_key_exists('auto_increment', $options) && $options['auto_increment'] === \true) {
            $sql .= ' auto_increment';
        }
        if (\array_key_exists('default', $options) && $options['default'] !== null) {
            if ($this->is_sql_method_call($options['default'])) {
                //$default_value = $options['default'];
                throw new \YoastSEO_Vendor\Ruckusing_Exception("MySQL does not support function calls as default values, constants only.", \YoastSEO_Vendor\Ruckusing_Exception::INVALID_ARGUMENT);
            }
...
//PRIVATE METHODS
    ...
    private function db_connect($dsn)
    {
        ...
        if ($db_info) {
            ...
            $this->conn = new \mysqli($db_info['host'], $db_info['user'], $db_info['password'], '', $db_info['port'], $db_info['socket']);
            //db name leaved for selection
