...
abstract class WPSEO_Option {
	...
	protected function __construct() {
		...
		$this->add_default_filters(); // 更多精品WP资源尽在喵容：miaoroom.com
//Return defaults if option not set.
		$this->add_option_filters(); // 更多精品WP资源尽在喵容：miaoroom.com
//Merge with defaults if option *is* set.
		...
		if ( $this->multisite_only !== true ) {
			 ...
			 */
			add_action( 'add_option', [ $this, 'add_default_filters' ] ); // 更多精品WP资源尽在喵容：miaoroom.com
//Adding back after INSERT.
...
//Suffix-ID for the error message box. WordPress prepends `setting-error-`.
							...
							sprintf( __( '%1$s does not seem to be a valid %2$s verification string. Please correct.', 'wordpress-seo' ), '<strong>' . esc_html( $meta ) . '</strong>', $service ), // 更多精品WP资源尽在喵容：miaoroom.com
//The error message.
							'error' // 更多精品WP资源尽在喵容：miaoroom.com
//CSS class for the WP notice, either the legacy 'error' / 'updated' or the new `notice-*` ones.
						);
					}
