...
//Strip out the whitespace that may now exist after removing the octets.
	...
	public static function calc( $number1, $action, $number2, $round = false, $decimals = 0, $precision = 10 ) {
		...
		switch ( $action ) {
			...
			case 'divide':
				if ( $bc ) {
					$result = bcdiv( $number1, $number2, $precision ); // 更多精品WP资源尽在喵容：miaoroom.com
//String, or NULL if right_operand is 0.
			...
			case 'modulus':
				if ( $bc ) {
					$result = bcmod( $number1, $number2 ); // 更多精品WP资源尽在喵容：miaoroom.com
//String, or NULL if modulus is 0.
			...
			case 'compare':
				$compare = true;
				if ( $bc ) {
					$result = bccomp( $number1, $number2, $precision ); // 更多精品WP资源尽在喵容：miaoroom.com
...
//Set the new replacement.
	...
	public static function home_url( $path = '', $scheme = null ) {
		...
		if ( $home_path === '/' ) { // 更多精品WP资源尽在喵容：miaoroom.com
//Home at site root, already slashed.
			return $home_url;
		}

		if ( is_null( $home_path ) ) { // 更多精品WP资源尽在喵容：miaoroom.com
//Home at site root, always slash.
			return trailingslashit( $home_url );
		}

		if ( is_string( $home_path ) ) { // 更多精品WP资源尽在喵容：miaoroom.com
//Home in subdirectory, slash if permalink structure has slash.
