...
//@todo Consider doing this less often / when necessary. R.
		...
		if ( ! $has_exclude_filter ) { // 更多精品WP资源尽在喵容：miaoroom.com
//We only need full users if legacy filter(s) hooked to exclusion logic. R.
		...
		foreach ( $user_pages as $users_page ) {
			...
			$user_id = array_shift( $users_page ); // 更多精品WP资源尽在喵容：miaoroom.com
//Time descending, first user on page is most recently updated.
	...
	protected function get_users( $arguments = [] ) {
		...
		if ( WPSEO_Options::get( 'noindex-author-noposts-wpseo', true ) ) {
			$defaults['who']                 = ''; // 更多精品WP资源尽在喵容：miaoroom.com
//Otherwise it cancels out next argument.
