...
if ( ! class_exists( 'All_in_One_SEO_Pack_Sitemap' ) ) {
	...
	class All_in_One_SEO_Pack_Sitemap extends All_in_One_SEO_Pack_Module {
		...
		public function get_sitemap_index_filenames() {
			...
			if ( ! empty( $options[ "{$this->prefix}taxonomies" ] ) ) {
				foreach ( $options[ "{$this->prefix}taxonomies" ] as $v1_taxonomy ) {
					...
					if( 
						isset( $aioseop_options[ "aiosp_${tax_name}_noindex" ] ) && 
					...
					if( 
						isset( $aioseop_options['aiosp_tax_noindex'] ) &&
		...
		public function get_date_archive_data() {
			...
			if ( is_array( $posts ) ) {
				foreach ( $posts as $p ) {
					...
					if ( empty( $date_archives[ $date ] ) ) {
						$date_archives[ $date ] = $p;
					} else {
						if ( $p->post_modified > $date_archives[ $date ]->post_modified ) {
							$date_archives[ $date ] = $p;
						}
					}
				}
		...
		public function get_author_archive_data() {
			...
			if ( ! empty( $author_archives ) ) {
				$priority  = $this->get_default_priority( 'archive' );
				$frequency = $this->get_default_frequency( 'archive' );

				if ( isset( $this->options[ $this->prefix . 'prio_author' ] ) && 'no' !== $this->options[ $this->prefix . 'prio_author' ] ) {
					if ( 'sel' !== $this->options[ $this->prefix . 'prio_author' ] ) {
						$priority = $this->options[ $this->prefix . 'prio_author' ];
					}
				}

				if ( isset( $this->options[ $this->prefix . 'freq_author' ] ) && 'no' !== $this->options[ $this->prefix . 'freq_author' ] ) {
		...
		public function get_tax_args( $taxonomies, $page = 0 ) {
			...
			$noindexed_tax_terms = array_merge( $noindexed_tax_terms, get_terms( 
				array( 
