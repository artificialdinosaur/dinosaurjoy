...
class SFWD_Update_Checker extends PluginUpdateChecker_3_1 {
	...
	function key_warning() {
		...
		if ( !$this->check_license_key( $license_key ) )
			$msg = "<p><strong>" . sprintf( __( '%s is almost ready.', 'all-in-one-seo-pack' ), $this->plugin_name ) . "</strong> "
		...
		$msg = '';
		if ( $this->has_update() ){
			$msg = '<p><strong>' . sprintf( __( "There is a new version of %1\$s available. Go to <a href='%2\$s'>the plugins page</a> for details.", 'all-in-one-seo-pack' ), AIOSEOP_PLUGIN_NAME, network_admin_url( 'plugins.php' ) ) . '</strong></p>';
			}
