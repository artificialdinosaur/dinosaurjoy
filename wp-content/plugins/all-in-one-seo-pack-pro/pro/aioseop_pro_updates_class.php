...
class aioseop_pro_updates {
	...
	function do_version_updates( $old_version ) {
		...
		*/
		...
		if ( ( AIOSEOPPRO && version_compare( $old_version, '3.4.0', '<' ) ) ) {
			// Video Sitemap cron scan needs a reset to repopulate data that was missing.
			$this->reset_video_sitemap_scan();
		}
	}
	...
	/**
	 * Reset Video-Sitemap Scan
	 *
	 * @since 3.4.0
	 */
	public function reset_video_sitemap_scan() {
		global $wpdb;
		$wpdb->query("DELETE FROM {$wpdb->postmeta} WHERE {$wpdb->postmeta}.meta_key = '_aioseop_video_sitemap_scan_at_modified_time'");
		...
		while ( false !== $next_scan_timestamp ) {
