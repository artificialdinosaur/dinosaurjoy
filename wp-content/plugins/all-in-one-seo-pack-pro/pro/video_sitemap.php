...
if ( class_exists( 'All_in_One_SEO_Pack_Sitemap' ) && ( !class_exists( 'All_in_One_SEO_Pack_Video_Sitemap' ) ) ) {
	class All_in_One_SEO_Pack_Video_Sitemap extends All_in_One_SEO_Pack_Sitemap {
		...
		function __construct( ) {
			...
			add_action( 'transition_post_status', array( $this, 'add_post_status_transition_hooks' ), 10, 3 );
			...
			add_action( 'aiosp_video_sitemap_scan', array( $this, 'scan_all_posts' ) );
			add_action( 'admin_notices', array( $this, 'scan_admin_notice' ) );
			add_action( $this->prefix . 'settings_update', array( $this, 'do_sitemaps' ) );
			add_filter( $this->prefix . 'update_options', array( $this, 'flush_video_scan_cache' ), 1 );
		...
		/**
		 * Add Post Status Transition Hooks
		 *
		 * Used to add hooks for publishing/updating a post.
		 * Excludes auto-saving. Data should not be updated until the user clicks Update on the post.
		 *
		 * @since 3.4.0
		 *
		 ...
		 * @param WP_Post $post
		 */
		public function add_post_status_transition_hooks( $new_status, $old_status, $post ) {
			// Autosave status.
			// Don't update data while it's being edited. Only when the user triggers a post transition.
			if ( 'inherit' === $new_status ) {
				return;
			}

			add_filter( 'post_updated', array( $this, 'scan_post' ) );
		}
		...
		public function cron_jobs() {
			if ( ! wp_next_scheduled( 'aiosp_video_sitemap_scan' ) ) {
				wp_schedule_event( time() + 10, 'daily', 'aiosp_video_sitemap_scan' );
			}
		}
		 ...
		 *
		 * Activation occurs with the Module Manager. This has no relation with plugin activation.
		 *
		 * @since ?
		 */
		public function activate_module() {
			...
			if ( false === $next_scan_timestamp ) {
				...
				set_transient( 'aiosp_video_sitemap_scan', 'on' );
				// show a notification.
		...
		/**
		 * Flush Scan Videos Cache.
		 *
		 * @since 3.3.5
		 *
		 * @param array $options New plugin options being saved.
		 * @return array Plugin options.
		 */
		public function flush_video_scan_cache( $options ) {
			$class_options = $this->get_class_option();

			if ( $options['aiosp_video_sitemap_posttypes'] !== $class_options['aiosp_video_sitemap_posttypes'] ) {
				delete_transient( 'aioseop_video_sitemap_scan_last_post_modified_timestamp' );
		 ...
		 *
		 * @since ?
		 * @since 3.3.5 Limit results to modified posts since last scan.
		 */
		public function scan_all_posts() {
			set_time_limit(0);
			// Schedule (possible) next scan in case of failed instance. Remove after scan_post(s),
			// and if post_count doesn't cap posts_limit.
			wp_schedule_single_event( time() + 10, 'aiosp_video_sitemap_scan' );

			/**
			 * Video Sitemap Scan's Post Limit
			 *
			 * Sets the amount of posts to query per scan.
			 *
			 * @since 3.3.5
			 *
			 * @param int $scan_posts_limit
			 */
			$scan_posts_limit = apply_filters( 'aioseop_video_sitemap_scan_posts_limit', 20 );
			$scan_posts_limit = intval( $scan_posts_limit );
			$query_args = array(
				'fields'         => 'all',
				'numberposts'    => $scan_posts_limit,
				'posts_per_page' => $scan_posts_limit,
				'orderby'        => 'post_modified_gmt',
				'order'          => 'ASC',
			);

			$last_post_modified = get_transient( 'aioseop_video_sitemap_scan_last_post_modified_timestamp ' );
			if ( $last_post_modified ) {
				$query_args['date_query'] = array(
					array(
						'after'  => date( 'Y-m-d H:i:s', $last_post_modified ),
						'column' => 'post_modified_gmt',
					),
				);
			}

			$query = $this->get_post_query( $query_args );

			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					$query->the_post();
					$this->scan_post( $query->post->ID );

					// Store the post's modified time to query the next scan after.
					$last_post_modified = strtotime( $query->post->post_modified_gmt );
					set_transient( 'aioseop_video_sitemap_scan_last_post_modified_timestamp', $last_post_modified, ( 3 * MONTH_IN_SECONDS ) );
				}
			}

			// If the Scan Posts Limit is not capped, indicating these are the last posts, then clean cron jobs.
			if ( $scan_posts_limit > $query->post_count ) {
				// Remove all scheduled events less than 12 hrs to reduce flooding cron jobs.
				// If daily cron is removed, then the self::cron_jobs() will reinstate the daily cron job.
				$next_scan_timestamp = wp_next_scheduled( 'aiosp_video_sitemap_scan' );
				while (
						false !== $next_scan_timestamp &&
						( $next_scan_timestamp - time() ) < ( 12 * HOUR_IN_SECONDS )
				) {
					wp_unschedule_event( $next_scan_timestamp, 'aiosp_video_sitemap_scan' );
					$next_scan_timestamp = wp_next_scheduled( 'aiosp_video_sitemap_scan' );
				}
		 ...
		 * 
		 * @since 3.3.0
		 * @since 3.3.5 Add support for WordPress supported video formats - #574
		 *
		...
		private function filter_links( $links ) {
			$patterns = array(
				...
				'#.*youtube.com/.*#',
				'#.*youtu.be/.*#',
				// Supported video formats - https://developer.wordpress.org/themes/functionality/media/video/#video
				'#.*.mp4#',
				'#.*.m4v#',
				'#.*.webm#',
				'#.*.ogv#',
			...
			/**
			 * Allows users to add or remove regex patterns that are used to detect supported video links. 
			 * 
			 ...
			 * @param array
			 */
			$patterns = apply_filters( 'aioseop_pro_supported_video_links', $patterns ) ;

			$filtered_links = array();
			foreach( $patterns as $pattern ) {
				$results = preg_grep( $pattern, $links );
		...
		function parse_video_opts( $data, $return_single = false ) {
			...
			if ( $links ) {
				foreach ( $links as $index => $link ) {
					$parse_url = wp_parse_url( str_replace( ':////', '://', esc_url_raw( $link ) ) );
					...
					$query_params = array();
					parse_str( wp_parse_url( $link, PHP_URL_QUERY ), $query_params );
		...
		/**
		 * oEmbed discover provider with URL.
		 *
		 * @since ?
		 *
		 * @param string     $url URL to discover oEmbed
		 * @param int|string $id  ? Used with `self::parse_video_opts()`.
		 * @return array
		 */
		function oembed_discover_url( $url, $id ) {
			...
			if ( !empty( $url ) ) {
				...
				$parse_url = wp_parse_url( str_replace( ':////', '://', esc_url_raw( $url ) ) );
		...
		function oembed_discovery( $html, $url, $c, $id, $data=null ) {
			...
			if ( !empty( $info ) ) {
				$opts[ $url ] = $info;
				if ( false === add_post_meta( $id, '_aioseop_oembed_info', $opts, true ) ) {
					$result = update_post_meta( $id, '_aioseop_oembed_info', $opts );
				}
		...
		/**
		 * Checks whether the given post contains videos and stores oembed info of videos in the postmeta table.
		 *
		 * @since ?
		 * @since 3.3.5 Skip posts that haven't been modified since last scan.
		 *
		 * @param int|WP_Post $post_id The ID of the post, or WP_Post on cron.
		 **/
		function scan_post( $post ) {
			...
			$scan_last_modified_time = get_post_meta( $post_obj->ID, '_aioseop_video_sitemap_scan_at_modified_time', true );
			if ( false === $scan_last_modified_time || strtotime( $post_obj->post_modified_gmt ) > strtotime( $scan_last_modified_time ) ) {
				...
				if ( ! empty( $post_obj ) ) {
					$html  = $this->get_post_content( $post_id, $post_obj );
					$parse = $this->parse_video_opts( array( 'id' => $post_id, 'html' => $html ) ); // try to detect manual embed codes
					if ( ! empty( $parse ) ) {
						foreach( $parse as $datum ) {
							if ( ! empty( $datum['video:player_loc'] ) ) {
								$this->oembed_discovery( $html, $datum['video:player_loc'], null, $post_id, $datum );
							}
						}
					}
				}

				update_post_meta( $post_obj->ID, '_aioseop_video_sitemap_scan_at_modified_time', $post_obj->post_modified_gmt );
			}
		}
